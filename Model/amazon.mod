...
sum { j in Routes: k = Route_SC[j] } X[j] <= 100*Y[k];
# SC[j] より、最大１００個までルートが作れる

## 行き先のルートによるカバー
JP_cover {k in JP_Sites}:
sum { j in Routes: k in Route_Sites[j] or k = Route_SC[j] } X[j] >= 1;

## 最大SC数  <=== 最大SC数の制約を入れる
SC_count: sum {k in SC_Sites} Y[k] <= Max_SC;
